            ...
            if (object != null)
            {
                ...
                for (int i = start; i < count; i++)
                {
                    if (i <= news.size() - 1)
                    {
                        ...
                        final LinearLayout newsContainer = (LinearLayout) rootView.findViewById(R.id.newsContainer);
                        final View newsItem = inflater.inflate(R.layout.news_container, null);
                        newsItem.setAnimation(Utils.animationSet());
                        NewsInfoFragment newsInfoFragment = new NewsInfoFragment();
                        newsInfoFragment.setNewsInfo(news.get(i));
                        fragmentTransaction.add(R.id.newsContainer1, newsInfoFragment);
